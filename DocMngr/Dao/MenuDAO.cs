    ...
    public class MenuDAO : BaseDAO
    {
        ...
        public List<MenuBean> getListMainMenu(string user)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                cmd.CommandText = "SELECT m.id FROM menu m " +
                                    ...
                                    "or (m.code in (select mr.menu_code from menu_role mr " +
                                    "               where mr.role_code in (select r.Code from aspnet_UsersInRoles uir, aspnet_Users u, aspnet_Roles r  " +
                                    "                                        where u.UserId=uir.UserId and u.LoweredUserName=@userName and uir.RoleId = r.RoleId))) " +
        ...
        private List<MenuBean> loadChildrens(int menuId, string user)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                SqlDataReader rd = null;
                cmd.CommandText = "SELECT m.id FROM menu M " +
                                    "WHERE M.active > 0 and M.master_id=@masterId " +
                                    "and ((m.id in (select mr.menu_id from menu_role mr " +
                                    "where role_id in (select uir.RoleId from aspnet_UsersInRoles uir, aspnet_Users u " +
                                     "where u.UserId=uir.UserId and u.LoweredUserName=@userName))) " +
                                     "or (m.code in (select mr.menu_code from menu_role mr where mr.role_code in (select r.Code from aspnet_UsersInRoles uir, aspnet_Users u, aspnet_Roles r  " +
                                     "where u.UserId=uir.UserId " +
                                        "and u.LoweredUserName=@userName " +
                                        "and uir.RoleId = r.RoleId))))  order by m.[order]";
