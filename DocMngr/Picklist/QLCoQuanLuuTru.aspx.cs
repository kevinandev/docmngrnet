...
namespace Picklist
{
    public partial class QLCoQuanLuuTru : System.Web.UI.Page
    {

        CoQuanLuuTru sec;
        Su_CoQuanLuuTruLogic um = new Su_CoQuanLuuTruLogic();
        string SecID = "";
        static string ID_Delete = "";
        string classobject = "QLCoQuanLuuTru.aspx.cs";
        ...
        protected void editApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in dgvApprover.Rows)
                {
                    ...
                    {
                        Session[Su_CoQuanLuuTruLogic.SESSION_SEC_ID] = ID;
        ...
        protected void btAddApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    else if (um.validateSecCode4Update(sec.Code, sec.ID))
                    {
                        um.updateCoQuanLuuTru(sec);
                        tbxCode.Text = "";
                        tbxName.Text = "";
                    ...
                    {
                        Logger.logmessage(classobject, "btAddApprover_Click", "Validate fail 4 update");
                    }
        ...
        private bool isUpdate()
        {
            if (Session[Su_CoQuanLuuTruLogic.SESSION_SEC_ID] != null)
            {
                SecID = Session[Su_CoQuanLuuTruLogic.SESSION_SEC_ID].ToString();
                return true;
            }
            return false;
        }
        ...
        protected bool validateSecurity(CoQuanLuuTru sec)
        {
            bool result = true;
            ...
            if (!um.validateSecCode(sec.Code.ToString()))
            {
                result = false;
                Response.Write("<script language='javascript'> { alert('Mã đơn vị này  đã có trong hệ thống. Xin chọn một mức khác khác');}</script>");
            }           
            return result;
        }
