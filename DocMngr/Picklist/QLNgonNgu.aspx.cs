...
namespace Picklist
{
    public partial class QLNgonNgu : System.Web.UI.Page
    {
        NgonNgu sec;
        Su_NgonNguLogic um = new Su_NgonNguLogic();
        string SecID = "";
        static string ID_Delete = "";
        string classobject = "QLNgonNgu.aspx.cs";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                try
                {
                    if (Session[Su_NgonNguLogic.SESSION_SEC_ID] != null)
                    {

                        btAddApprover.Text = "Cập nhật";
                        sec = um.getNgonNgu(Int32.Parse(Session[Su_NgonNguLogic.SESSION_SEC_ID].ToString()));
                        if (sec != null)
                        {
            ...
            else
            {
                ...
                if (Session[Su_NgonNguLogic.SESSION_SEC_ID] != null)
                {
                    tId = Session[Su_NgonNguLogic.SESSION_SEC_ID].ToString();
                    sec = new NgonNgu();
                    int iId = 0;
        ...
        protected void editApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in dgvApprover.Rows)
                {
                    ...
                    {
                        Session[Su_NgonNguLogic.SESSION_SEC_ID] = ID;
                        Response.Redirect("QLNgonNgu.aspx", false);
                    }
        ...
        protected void btAddApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Session[Su_NgonNguLogic.SESSION_SEC_ID] = null;
                Response.Redirect("QLNgonNgu.aspx", false);
            }
            catch (Exception ex)
            {
        ...
        private bool isUpdate()
        {
            if (Session[Su_NgonNguLogic.SESSION_SEC_ID] != null)
            {
                SecID = Session[Su_NgonNguLogic.SESSION_SEC_ID].ToString();
                return true;
            }
            return false;
        }
        ...
        protected bool validateSecurity(NgonNgu sec)
        {
            bool result = true;
