...
namespace Picklist
{
    public partial class QLMucDoTinCay : System.Web.UI.Page
    {
        MucDoTinCay sec;
        Su_MucDoTinCayLogic um = new Su_MucDoTinCayLogic();
        string SecID = "";
        static string ID_Delete = "";
        string classobject = "QLMucDoTinCay.aspx.cs";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                try
                {
                    if (Session[Su_MucDoTinCayLogic.SESSION_SEC_ID] != null)
                    {

                        btAddApprover.Text = "Cập nhật";
                        sec = um.getMucDoTinCay(Int32.Parse(Session[Su_MucDoTinCayLogic.SESSION_SEC_ID].ToString()));
                        if (sec != null)
                        {
            ...
            else
            {
                ...
                if (Session[Su_MucDoTinCayLogic.SESSION_SEC_ID] != null)
                {
                    tId = Session[Su_MucDoTinCayLogic.SESSION_SEC_ID].ToString();
                    sec = new MucDoTinCay();
                    int iId = 0;
        ...
        protected void editApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in dgvApprover.Rows)
                {
                    ...
                    {
                        Session[Su_MucDoTinCayLogic.SESSION_SEC_ID] = ID;
                        Response.Redirect("QLMucDoTinCay.aspx", false);
                    }
        ...
        protected void btAddApprover_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!isUpdate())
                {
                    ...
                    {
                        um.addMucDoTinCay(sec);
                        tbxDescription.Text = "";
                        tbxName.Text = "";
                    }
                    else
                    {
                        Logger.logmessage(classobject, "btAddApprover_Click", "Validate fail 4 inssert " + sec.Name);
                    }
                ...
                Session[Su_MucDoTinCayLogic.SESSION_SEC_ID] = null;
                Response.Redirect("QLMucDoTinCay.aspx", false);
            }
            catch (Exception ex)
            {
        ...
        private bool isUpdate()
        {
            if (Session[Su_MucDoTinCayLogic.SESSION_SEC_ID] != null)
            {
                SecID = Session[Su_MucDoTinCayLogic.SESSION_SEC_ID].ToString();
                return true;
            }
            return false;
        }
        ...
        protected bool validateSecurity(MucDoTinCay sec)
        {
            bool result = true;
